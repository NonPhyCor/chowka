The Chosen One is a simple, easy to use, open source web based application to manage user assets (both physical and digital), using shamir's secret sharing and node.js crypto packages for securing and encrypting user data. Supabase powers the database for the website.

The application's capabilities include:
 1. Generating digital wills of intent that are downloadable.
 2. Generating emails containing user codes
 3. Securely managing sensitive information using shards on sss and node.js crypto.

The Chosen One is a platform independent, open source application deployed on vercel and can be accessed at https://chowka.vercel.app and is compatible with any web browser.

File Structure:
Repogenesis/chowka<br>
|<br>
|_script.js<br>
|_styles.css<br>
|_index.html<br>
|_readme.md<br>
